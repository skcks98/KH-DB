...
-- 2024-10-08 11:09:11.000
-- SYSDATE : 시스템상 현재 시간을 나타내는 함수
...
-- DISTINCT : 조회 시 칼럼에 포함된 중복 값을 한번만 표기
-- 주의사항 1) DISTINCT 구문은 SELCET 마다 딱 한번씩만 표기 가능
-- 주의사항 2) DISTINCT 구문은 SELCET 제일 앞에 작성되야한다.ㄴ

SELECT DISTINCT DEPT_CODE, JOB_CODE FROM EMPLOYEE;

-------------------------------------------------------

--1. SELECT 절 : SELECT 컬럼명
--2. FROM 절 : FROM 테이블명
--3. WHERE 절 : WHERE 컬럼명 연산자 값;
--4. ORDER BY 컬럼명 | 별칭 | 컬럼순서 [ASC | DESC]

-- EMPLOYEE 테이블에서 급여가 3백만 초과인 사원의 사번 이름, 급여, 부서코드를 조회
...
-- EMPLOYEE 테이블에서
-- 이메일 '_' 앞이 4글자 이면서
-- 부서코드가 'D9' 도는 'D6' 이고 --> AND가 OR보다 우선순위가 높다. ( ) 사용!
