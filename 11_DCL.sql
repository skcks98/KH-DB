...
CREATE USER ncw_sample IDENTIFIED BY 1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ...  TO 사용자명;
GRANT CREATE SESSION TO ncw_sample; 
...
-- CREATE TABLE : 테이블 생성 권한
-- + 데이터 저장할 수 있는 공간 (TABLESPACE) 할당

-- 6.(SYS) 테이블 생성 권한 부여 + TABLESPACE 할당

GRANT CREATE TABLE TO ncw_sample;

ALTER USER ncw_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
GRANT CONNECT, RESOURCE TO ncw_sample; 
...
SELECT * FROM KH_NCW.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 

-- 2. (KH) sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON EMPLOYEE TO ncw_sample
GRANT SELECT ON EMPLOYEE TO ncw_sample;
...
REVOKE SELECT ON EMPLOYEE FROM ncw_sample; -- 권한 회수
