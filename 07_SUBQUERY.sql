...
-- 1) 노옹철의 부서코드 조회 (서브 쿼리)
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE EMP_NAME ='노옹철'; -- 'D9'

-- 2) 부서코드가 'D9'인 직원의 이름, 부서코드를 조회(메인 쿼리)
...
WHERE DEPT_CODE = (SELECT DEPT_CODE
					FROM EMPLOYEE
					WHERE EMP_NAME ='노옹철'); -- 서브 쿼리를 이용한 SELECT 문
...
-- 1) 사수에 해당하는 사원번호 조회 (서브 쿼리)
SELECT DISTINCT MANAGER_ID  -- 중복 제거 DISTINCT
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
	CASE WHEN EMP_ID IN (SELECT DISTINCT MANAGER_ID  -- 중복 제거 DISTINCT
						FROM EMPLOYEE
						WHERE MANAGER_ID IS NOT NULL)
		THEN '사수'
		ELSE '사원'
...
-- 차장 직급 급여중 가장 큰값보다 많이 받는 과장 직급의 직원
-- 사번, 이름, 직급, 급여 조회
...
-- 3) EMPLOYEE테이블에서 위의 결과들과 동일한 DEPT_CODE를 가진 사원을 조회
SELECT EMP_NAME, DEPT_CODE	
FROM EMPLOYEE
WHERE DEPT_CODE  IN (SELECT DEPT_ID -- 다중 행
					FROM DEPARTMENT
					WHERE LOCATION_ID = (SELECT LOCAL_CODE
											FROM LOCATION
											WHERE NATIONAL_CODE ='KO')); -- 단일 행
...
-- 2) 퇴사한 여직원과 같은 부서, 직급 조회

-- 방법 1) 단일행 서브쿼리 2개를 사용해서 조회
...
-- 사번, 이름, 직급코드, 급여 조회
-- 단, 급여와 급여 평균은 만원단위로 계산 TRUNC(칼럼명, -4)
...
-- RANL() OVER : 동일한 순위 이후의 등수를 동일한 인원 수 만큼 건너 뛰고 순위 계산.
...
JOIN JOB USING(JOB_CODE) -- USING, NATURAL 애매해서 잘 안씀
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, TO_CHAR(HIRE_DATE, 'RR/MM/DD') HIRE_DATE 
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE) = ( SELECT DEPT_CODE, JOB_CODE
							    FROM EMPLOYEE
							    WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000);
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, MANAGER_ID, EMP_NO, TO_CHAR(HIRE_DATE, 'RR/MM/DD')  HIRE_DATE 
FROM EMPLOYEE
WHERE (DEPT_CODE, MANAGER_ID) = (SELECT DEPT_CODE, MANAGER_ID
									FROM EMPLOYEE
									WHERE EMP_NO LIKE '77%'
...
-- SQL 문법 규칙 : USING 절은 JOIN을 할 때 특정 컬럼을 명시할때만 사용됨
