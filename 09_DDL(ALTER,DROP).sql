...
--> NOT NULL 제약조건은 서로 조건을 추가하는것이 아닌
-- 컬럼자체에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식됨


-- MODIFY(수정하다) 구문을 사용해서 NULL 제어
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; --DEPT_TITLE 칼럼에 NULL 비허용



ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE  NULL; -- DEPT_TITLE 칼럼에 NULL 허용
...
SELECT * FROM  DEPT_COPY;
-- 칼럼이 생성되면서 DEFAULT 값이 자동 삽입되었음을 확인
...
-- SQL Error [12899] [72000]: ORA-12899: "KH_NCW"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 데이터 타입이 CHAR2() 이므로 영어 숫자 2글자까지 저장 가능
--> D10 은 CHAR(3) 에 들어올수 있음
--> VARCHAR2(3) 으로 바꾸기 (남는 바이트 메모리 반환을 위배)
...
-- DEPARTMENT 테이블 복사해서 DEPT_COPY

CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT;
--> 컬럼명, 데이터타입, NOT NULL 여부만 복사

-- DEPT_COPY 테이블에 PK 추가 ( 컬럼 : DEPT_ID, 제약 조건명 ㅣ D_COPY_PK)
...
-- 3) DROP TABLE 삭제 옵션 CASCADE CONSTRAINT 사용
	--> CASCADE CONSTRAINT : 삭제하려는 테이블과 연결된 FK 제약조건을 모두 삭제


DROP TABLE TB1 CASCADE CONSTRAINT; -- 테이블 삭제 시 FK 관계도 삭제
...
--> DDL이 종료돈 후 DML 구문을 수행할 수 있도록 권장!
--
...
ROLLBACK;
SELECT * FROM TB2;
-- 롤백안된다.. 위에서 DDL구문 중 ALTER를 사용해서 그 시점에 COMMIT 됨
