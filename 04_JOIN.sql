...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- 내부 조인
...
-- NULL값은 마케팅부, 국내영업부, 해외영업3부에 사원이 없다
...
-- 테이블마다 별칭 작성(미작성시 열의정의가 애매하다는 오류 발생)
...
-- LEFT, RIGHT JOIN은 기준이 되는 테이블 반대편에 (+)를 달아줌 / 지금은 LEFR JOIN
...
AND DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') = '여'
AND EMP_NAME LIKE '전%';
...
SELECT EMP_NAME 사원명, JOB_NAME 직급명, DEPT_TITLE 부서코드, DEPT_ID 부서명
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE DEPT_TITLE IN('해외영업1부', '해외영업2부')
AND JOB_NAME LIKE '사원';
...
-- 5. 부서가 있는 사원의 사원명, 직급명, 부서명, 지역명 조회 NULL 2명
