...
SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래 절삭
SELECT TRUNC(123.456, 1) FROM DUAL; --123.4, 소수점 첫째자리 아래 절삭
SELECT TRUNC(123.456, -1) FROM DUAL; -- 120, 10의자리  절삭
...
-- TO_CHAR(날짜, [포맷]) : 숫자형 데이터를 문자형 데이터로 변경

-- 숫자 -> 문자 변환시 포맷 패턴
-- 9 : 숫자 한칸을 의미, 여러개 작성 시 오른 쪽 정렬
-- 0 : 숫자 한칸을 의미, 여러개 작성 시 오른 쪽 정렬 + 빈칸 0 추가
-- L : 현재 DB에 설정된 나라의 화폐 기호

SELECT TO_CHAR(1234, '99999') FROM DUAL; -- ' 1234' 9패턴
SELECT TO_CHAR(1234, '00000') FROM DUAL; -- '01234' 0패턴
SELECT TO_CHAR(1234) FROM DUAL; -- '1234' 포맷은 옵션!
...
-- TO_DATE(문자형, [포맷]) : 문자형 -> 날짜로 변경
-- TO_DATE(숫자형, [포맷]) : 숫자형 -> 날짜로 변경
--> 지정된 포맷으로 날짜를 인식함.
...
-- R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우는 이전세기(1900)
...
-- EMPLOYEE 테이블에서 보너스를 받으면 '0', 안바드면 'X' 조회
...
-- 급여 최대값, 가장느린 입사일, 알파벳순서가 가장느린 이메일 조회
...
-- SELECT MAX(SALART) FROM EMPLOYEE
-- 서브쿼리 + 그룹함수

-- COUNT : 행 개수를 헤아려서 리턴
-- COUTN(컬럼명) : NULL을 제외한 실제값이 기록된 행 개수를 리턴
